...
def add_to_watchlist(user_id, film_id, public=True):
    ...
    Args:
        ...
        public (bool, optional): Whether the entry is publicly visible.
            Defaults to True (see Comment 4 in pr-response.md); callers can
            pass public=False to add a film privately.
...
def remove_from_watchlist(user_id, film_id):
    ...
    Mirrors remove_from_collection() in the collection service.
...
def get_watchlist(user_id):
    ...
    Sort order matches get_collection() for consistency across the two list
    endpoints (see Comment 5 in pr-response.md).
